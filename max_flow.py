...
def calculate_average_speed(flow):
    # Sort the flow values based on magnitude (speed)
    speed = np.sqrt(flow[:, :, 0] ** 2 + flow[:, :, 1] ** 2)
    sorted_indices = np.argsort(speed.flatten())[::-1][:100]

    # Extract the top 100 flow vectors
    top_100_flow = flow.flatten()[sorted_indices].reshape(-1, 2)

    # Calculate the average speed from the top 100 flow vectors
    average_speed = np.mean(np.sqrt(top_100_flow[:, 0] ** 2 + top_100_flow[:, 1] ** 2))
...
def draw_flow_with_average_speed(img, flow, step=10):
    ...
    cv2.putText(img_bgr, f"Average Speed (Top 100): {average_speed_mps:.2f} cm/s", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
                (0, 0, 255), 2)
...
while True:
    ...
    average_speed_values.append(calculate_average_speed(flow))
    ...
    speed_history_equalizer.append(average_speed_values[-1])
